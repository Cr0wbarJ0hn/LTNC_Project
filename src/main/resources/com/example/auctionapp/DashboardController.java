...
public class DashboardController {
    ...
    public void loadDetailedBidScreen() {
        try {
            ...
            String price = currentBidLabel.getText().replace("$", "").replace(",", "");
    ...
    public void handleQuickBid() {
        // 1. Create the JavaFX Input Dialog
        TextInputDialog dialog = new TextInputDialog();
        dialog.setTitle("Place Quick Bid");
        dialog.setHeaderText("Bidding on: " + previewTitle.getText());
        dialog.setContentText("Enter your bid amount ($):");

        // 2. Show the pop-up and wait for the user to click "OK"
        Optional<String> result = dialog.showAndWait();

        // 3. If they clicked OK and typed something, process it
        result.ifPresent(inputString -> {
            try {
                double amount = Double.parseDouble(inputString.trim());

                // 4. Pack the network payload
                ...
                // 5. Send it on a background thread
                new Thread(() -> {
                    try {
                            ...
                            if ("BID_RESPONSE".equals(serverReply.action)) {
                                Platform.runLater(() -> {
                                    if (serverReply.success) {
                                        ...
                                        successAlert.setContentText("Your bid was placed successfully!");
                                        successAlert.show();
                                    } else {
                                        // Show an error alert (e.g., bid too low)
                ...
                alert.setContentText("Please enter a valid number (e.g., 150.50).");
                alert.showAndWait();
            }
        });
    }
